...
func outputMayorContext(ctx RoleContext) {
	...
	fmt.Println("- `" + cli.Name() + " bd ready` - Issues ready to work")
	fmt.Println()
...
func outputPolecatContext(ctx RoleContext) {
	...
	fmt.Println("4. If no mail, check `" + cli.Name() + " bd list --status=in_progress` for existing work")
	fmt.Println()
	fmt.Println("## Key Commands")
	fmt.Println("- `" + cli.Name() + " mail inbox` - Check your inbox for work assignments")
	fmt.Println("- `" + cli.Name() + " bd show <issue>` - View your assigned issue")
	fmt.Println("- `" + cli.Name() + " bd close <issue>` - Mark issue complete")
